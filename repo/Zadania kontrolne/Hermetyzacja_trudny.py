...
def przeliczWaluty(walutaWyjsciowa, walutaDocelowa, kwota):
    if walutaWyjsciowa != 'zł':
        kwota *= KURSY[walutaWyjsciowa]
    kwota /= KURSY[walutaDocelowa]
    return kwota
...
class KontoBankowe:
    ...
    def przelejPieniadze(self, konto, kwota):
        if self.wyplacKwote(kwota) == 1:
            kwota = przeliczWaluty(self.__waluta, konto.sprawdzWalute(), kwota)
...
konto2 = KontoBankowe('654321', 'dolar', '54321')

konto2.wplacKwote(5000)
konto2.sprawdzStanKonta()

#konto2.przeliczWaluty()
